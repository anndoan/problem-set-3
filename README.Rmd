...
cleaned_data  %>%
  ...
  select(-time.point, -time.value) %>% spread(time.hpi, value) %>% mutate(expr.diff = hpi.24 - hpi.6) -> expr_diff
expr_diff %>% group_by(plant, fungus, gene.name) %>%

              summarise(mean.expr = mean(expr.diff), 
                        var.expr = var(expr.diff)) -> expr_diff
expr_diff %>% arrange(desc(mean.expr)) %>% slice(1:3) %>% ungroup(gene.name) -> expr_diff
expr_diff
...
There are `r nrow(expr_diff)` rows and `r ncol(expr_diff)` of data in expr_diff. The plant/fungus pair `r expr_diff %>% arrange(desc(mean.expr)) %>% slice(1) %>% select(plant, fungus)` has the highest mean expression value which is  `r expr_diff %>% arrange(desc(mean.expr)) %>% slice(1) %>% select(mean.expr)` The lowest variance expression value is `r expr_diff %>% arrange(desc(var.expr)) %>% tail() %>% slice(6) %>% select (var.expr)`
...
plot_data <- semi_join(cleaned_data, expr_diff, by = 'gene.name') %>%
  mutate(time.value = str_replace(time.point, 'hpi', '')) %>%
  mutate(time.value = as.integer(time.value))

ggplot(plot_data, aes(x = time.value, y = value, color = factor(gene.name))) + geom_point(aes(shape = rep)) + geom_smooth(method = 'lm', se = FALSE) + facet_wrap(fungus~plant) + theme_bw() + ggtitle('The expression value of plant and fungus at different time points') 
```
