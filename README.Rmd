...
library(ggrepel)
library(cowplot)
ggplot(cleaned_data, aes(x = time.point, y = value, color = factor(gene.name))) + geom_point(aes(shape = rep)) + geom_smooth(method = 'lm', se = FALSE) + facet_wrap(fungus~plant) + theme_bw() + ggtitle('The expression value of plant and fungus at different time points') 
```
